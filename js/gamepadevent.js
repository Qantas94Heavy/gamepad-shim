...
define(['gamepad'], function (Gamepad) {
  ...
  function isObjectCoercible(arg) {
    // jshint eqnull:true
    return arg != null;
  }
  ...
  function GamepadEvent(type, eventInitDict) {
    ...
    if (!(this instanceof GamepadEvent)) throw new TypeError(
      "Failed to construct 'GamepadEvent': Please use the 'new' operator, " +
      'this DOM object constructor cannot be called as a function.'
    );
    
    if (!arguments.length) throw new TypeError(
      "Failed to construct 'GamepadEvent': An event name must be provided."
    );
    
    var eventBase = new Event(type, eventInitDict);
    if (isObjectCoercible(eventInitDict) && 'gamepad' in eventInitDict) {
      ...
      , value: eventInitDict.gamepad
        ...
        "Failed to construct 'GamepadEvent': The 'gamepad' property does not have a Gamepad type."
      );
    }
  ...
  if (canChangePrototype) {
    ...
    else Object.defineProperty(GamepadEventPrototype, 'toString', toStringDescriptor);
  }
  ...
  // TODO: make these double underscore
